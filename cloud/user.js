...
Parse.Cloud.define("getUserInfo", function (req, res) {
    ...
    userQuery.get(userId, { useMasterKey: true })
        .then(function (user) {
            if (user) {
                tools.success(req, res, 'get userInfo success', user);
            }
            else {
    ...
    tools.checkAdmin(req.user)
        .then(function (result) {
            ...
            if (!username || !firstName || !lastName) {
                tools.error(req, res, 'username, firstName, lastName was not undefine', errorConfig.REQUIRE);
                return;
            }
            if (!password && !id) {
                tools.error(req, res, ' password was not undefine', errorConfig.REQUIRE);
                return;
            }
                ...
                checkEmailExists(email)
                    .then(function (userExists) {
                        if (userExists && userExists.id != id) {
                            tools.error(req, res, 'email', errorConfig.INSIDE_MANY);
                            return;
                        }
                        else {
                            checkUserExists(username)
                                .then(function (userNameExist) {
                                    if (userNameExist && userNameExist.id != id) {
                                        tools.error(req, res, 'username', errorConfig.INSIDE_MANY);
                                        return;
                                    }
                                    else {
                                ...
                                .catch(function (err) {
                                    tools.error(req, res, 'check Username', errorConfig.ACTION_FAIL, err);
                                })
                        }
                    })
                    .catch(function (err) {
                        tools.error(req, res, 'check email', errorConfig.ACTION_FAIL, err);
                    })
            }
            else {
                checkUserExists(username)
